...
- **現象**: [758800ca](../../data/tasks.json) のタスク実行時に `FileNotFoundError` が発生。
...
`task process:all` 実行時、`apps/capture-vrchat/src/cli.py` が `ProcessRecordingUseCase` に `str` を渡していましたが、ユースケース側は `RecordingSession` オブジェクトを期待していました。
...
2.  **CLI の修正**: `apps/capture-vrchat/src/cli.py` でファイル名から日時を抽出し、正しい `RecordingSession` オブジェクトを生成して渡すよう変更。
