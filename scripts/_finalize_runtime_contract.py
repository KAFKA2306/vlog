...
'''
    ...
    text = text.replace("touch data/summaries/*.txt", 'touch "{{.VLOG_DATA_HOME}}"/summaries/*.txt')
...
'''
    ...
    text = text.replace("cmd: cd apps/reader && $BUN ci", "cmd: cd apps/reader && $BUN install --frozen-lockfile")
    text = text.replace('python3 infra/systemd/render.py --root "$(pwd)"', '$UV run --frozen python infra/systemd/render.py --root "{{.ROOT_DIR}}"')
...
def main() -> None:
    ...
    if remaining:
        raise RuntimeError("Taskfile finalization left forbidden tokens: " + ", ".join(remaining))
